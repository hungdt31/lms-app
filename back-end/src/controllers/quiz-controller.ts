...
export default class QuizController extends BaseController {
  ...
  public initializeRoutes() {
    ...
    this.router.post(`${this.path}` + '/question',
                      this.addQuestion);
    this.router.delete(`${this.path}` + '/question', 
                      this.deleteQuestion);
  ...
  */
  private createQuiz = asyncHandler(
    async (request: express.Request, response: express.Response) => {
      const quiz = await prisma.quiz.create({data: request.body})
  ...
  */
  private markQuiz = asyncHandler(
    ...
    for (let i = 0; i < solution.questions.length; i++) {
      console.log(solution.questions[i].answer + " " + request.body.answer[i] + '\n') 
